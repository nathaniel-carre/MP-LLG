...
(* Crée un graphe biparti G = (X U Y, A) aléatoire, tel
que |X| = nx, |Y| = ny et tous les sommets de X sont de
...
(* Crée des pondérations aléatoires de valeur ≤ poids_max
sous forme de matrice pour un graphe biparti donné en 
argument. *)
let creer_ponderations g poids_max graine =
    Random.init graine;
    let m = Array.make_matrix (g.nx + g.ny) (g.nx + g.ny) infinity in
    for x = 0 to g.nx - 1 do
        let ajout_poids y =
            let poids = Random.float poids_max in
            m.(x).(y) <- poids; m.(y).(x) <- poids in
        List.iter ajout_poids g.adj.(x)
    done;
    m;;
...
let m1 = creer_ponderations g1 30. 384;;
let m2 = creer_ponderations g2 100. 42;;